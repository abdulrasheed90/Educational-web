  ...
  const handleAnswer = async (isCorrect) => {
    ...
    if (currentQuestion && user) {
      try {
        ...
        // Silent operation - no user feedback needed for auto-save
      } catch (err) {
        // Silent error handling - answer tracking is non-critical
      }
    }
    ...
    return (
      <div className="min-h-screen py-20 px-4 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <button
            onClick={() => navigate(-1)}
            className="px-6 py-3 bg-primary-gradient rounded-xl transition-colors shadow-lg shadow-[#06b5cc]/10"
          >
            Go Back
          </button>
        </div>
    ...
    <div className="min-h-screen py-20 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div initial={{
          ...
          <div className="mt-4 h-2 bg-white/10 rounded-full overflow-hidden">
            <motion.div initial={{
              ...
              duration: 0.5
            }} className="h-full bg-gradient-to-r from-[#06b5cc] to-[#EADADA]" />
          </div>
        </motion.div>
        ...
        {isQuestionLocked ? <motion.div initial={{
          ...
          duration: 0.6
        }} className="bg-gradient-to-br from-[#111113] to-[#111113]/50 border border-white/10 rounded-2xl p-12 text-center">
          ...
          <Link to="/pricing" className="inline-block px-8 py-3 bg-gradient-to-r from-[#F7C94C] to-[#06b5cc] hover:opacity-90 rounded-xl transition-all duration-300">
            Upgrade to Premium
          </Link>
        </motion.div> : <>
          {/* Question Card */}
          <AnimatePresence mode="wait">
            <motion.div key={currentQuestionIndex} initial={{
              ...
              duration: 0.4
            }} className="bg-gradient-to-br from-[#111113] to-[#111113]/50 border border-white/10 rounded-2xl p-8 mb-8">
              <div className="mb-6">
                <h3 className="text-[#EADADA] mb-4">Question</h3>
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-[#EADADA]">Answer</h3>
                ...
                {showAnswer ? (
                  currentQuestion.isPremium && !isPremium ? (
                    <div className="relative">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center bg-[#111113]/80 rounded-xl">
                        <div className="text-center">
                          <Lock className="w-8 h-8 text-[#F7C94C] mx-auto mb-3" />
                          <p className="text-sm mb-3">Premium answer</p>
                          <Link to="/pricing" className="text-[#06b5cc] hover:text-[#EADADA] text-sm transition-colors">
                            Upgrade to unlock
                          </Link>
                        </div>
                      </div>
                    ...
                    >
                      ...
                      {(correctAnswerText || correctAnswerImage || answerImage) && (
                        <div className="bg-[#0A0E14] rounded-xl p-6 border border-[#06b5cc]/30">
                          {correctAnswerText && (
                            <p className="text-[#94A3B8]">
                              <strong className="text-[#EADADA]">Correct Answer:</strong> {correctAnswerText}
                            </p>
                          )}
                        </div>
                      )}
                      ...
                      {correctAnswerImage && (
                        <div className="bg-[#0A0E14] rounded-xl p-6 border border-[#06b5cc]/30">
                          <img
                            ...
                            alt="Correct answer"
                            className="rounded-lg w-full max-w-2xl mx-auto border border-[#06b5cc]/40"
                            style={{ maxHeight: '500px', maxWidth: '672px', width: '100%', height: 'auto', objectFit: 'contain' }}
                            onError={(e) => {
                              // Silently handle image load errors
                              e.target.style.display = 'none';
                      ...
                      {currentQuestion.hint && (
                        <div className="bg-[#0A0E14] rounded-xl p-6 border border-[#06b5cc]/30">
                          <p className="text-[#94A3B8]">
                            <strong className="text-[#EADADA]">Hint:</strong> {currentQuestion.hint}
                          </p>
                        </div>
                      )}
                    </motion.div>
                  ...
                  <button
                    ...
                    className="w-full px-6 py-3 bg-[#06b5cc]/10 hover:bg-[#06b5cc]/20 border border-[#06b5cc] rounded-xl transition-colors flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
                  >
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              {questions.slice(0, Math.min(questions.length, isPremium ? questions.length : freeQuestionLimit)).map((_, index) => <button key={index} onClick={() => {
                ...
                setShowAnswer(false);
              }} className={`w-10 h-10 rounded-lg transition-all ${index === currentQuestionIndex ? 'bg-primary-gradient text-white shadow-lg shadow-[#06b5cc]/20' : answers[index] === 'correct' ? 'bg-green-600/30 border border-green-600' : answers[index] === 'wrong' ? 'bg-red-600/30 border border-red-600' : 'bg-white/5 hover:bg-white/10 border border-white/10'}`}>
                {index + 1}
              </button>)}
            </div>

            <button onClick={nextQuestion} disabled={currentQuestionIndex === questions.length - 1 || currentQuestionIndex === freeQuestionLimit - 1 && !isPremium} className="px-6 py-3 bg-primary-gradient rounded-xl transition-colors shadow-lg shadow-[#06b5cc]/10 disabled:opacity-50 disabled:cursor-not-allowed">
              Next
            </button>
          </div>
        </>}
